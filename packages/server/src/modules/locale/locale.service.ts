...
import { LocaleListVO, LocaleVO } from './locale.vo';

import { RecordService } from '../record/record.service';
import { SettingService } from '../setting/setting.service';
import { transformListToObj, completeLocaleItem } from './locale.utils';
...
export class LocaleService {
  ...
  async addLocale(user, addLocaleDTO: AddLocaleDTO): Promise<LocaleVO> {
    ...
    const Locale = await this.localeRepository.findOne({ 'zh-CN': key });
    if (Locale) {
  ...
  async updateLocale(user, update: UpdateLocaleDTO): Promise<LocaleVO> {
    ...
    const toUpdate = await this.localeRepository.findOne({ 'zh-CN': key });
  ...
  async deleteLocale(user, data: DeleteLocaleDTO): Promise<DeleteResult> {
    ...
    // 增加操作记录
    const toDelete = await this.localeRepository.findOne({ 'zh-CN': key });
